...
export function CustomersList({ toggleActive }) {
  ...
  const searchData = async (value) => {
    ...
    if (response.ok) {
      ...
      if (searchCustomerData.length > 0) {
        ...
        //   rowSearchNotFound =
        //     search_customer.search_customer.customers.length == 0 &&
        //     search_customer.search_customer.customers.length == 0 ? (
        //       <CustomersNotFound />
        //     ) : (
        //       ""
  ...
  const debouncedSearch = debounce(searchData, debounceDelay);
  ...
  const mainIndex = (
    ...
    >
      {/* {data != undefined
        ? updatedRowMarkup
        : rowMarkup} */}
      {/* {filterValue != "" && filterCustomer.length > 0
        ? filterRowMarkup
        : value != "" && isSearching
        ? searchRowMarkup
        : // : isUpdated  ? mofidyRowMarkup :
        data != undefined
        ? updatedRowMarkup
        : rowMarkup} */}
      {/* {rowMarkup} */}

      {/* {isUpdated && updatedCustomer.length > 0 ? updatedRowMarkup : rowMarkup} */}
  ...
  return (
    <LegacyCard>
      <Frame>
        ...
        {/* {isFilter && taggedWith != "" && filterCustomer.length == 0 ? (
          <CustomersNotFound />
        ) : (
          mainIndex
        )} */}

        {/* {isSearching && searchRowMarkup.length == 0 ? (
          <div style={{ width: "100%" }}>
            <CustomersNotFound />
          </div>
        ) : queryValue != " " &&
        ...
        {/* { queryValue != "" && isSearching && searchCustomerData.length == 0 ? 
        (
          <CustomersNotFound />
        ) : 
        (
        ...
        {/* {isFilter && taggedWith != "" && filterProduct.length == 0 ? (
          <CustomersNotFound />
        ) : (
          <>
            {toastMarkup}
      ...
      {/* {isSearching && searchCustomerData.length >= 50 ? 
